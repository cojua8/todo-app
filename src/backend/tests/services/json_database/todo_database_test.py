...
def test_get_all_ok(setup_todos, setup_service):
    ...
    # act
    actual_todos = todo_service.get_all()
...
def test_get_returns_todo(setup_todos, setup_service):
    # arrange
    ...
    # act
    actual_todo = todo_service.get(expected_todo.id)
...
def test_get_returns_none(setup_todos, setup_service):
    # arrange
    todos, _ = setup_todos
    todo_service, io_service = setup_service
    io_service.read.return_value = json_utils.dumps(todos)
    ...
    # act
    actual_todo = todo_service.get(searched_id)

    # assert
    assert actual_todo is None


def test_create_ok(setup_service, todo_factory):
    # arrange
...
def test_delete_ok(setup_service, setup_todos):
    # arrange
    todos, expected_todo = setup_todos
    todo_service, io_service = setup_service
    io_service.read.return_value = json_utils.dumps(todos)
    ...
    # act
    todo_service.delete(expected_todo.id)
...
@pytest.mark.parametrize("setup_todos", [({}, 4)], indirect=True)
def test_put_ok(setup_todos, setup_service, todo_factory):
    # arrange
    todos, expected_todo = setup_todos
    ...
    # act
    todo_service.put(expected_todo.id, updated_todo)
...
def test_get_all_by_user_id_ok(setup_service, setup_todos, todo_factory):
    # arrange
    todos, expected_todo = setup_todos
    ...
    # act
    actual_todos = todos_service.get_all_by_user_id(owner_id)
