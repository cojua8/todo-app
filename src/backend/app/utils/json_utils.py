...
class _EnhanchedJSONDecoder(json.JSONDecoder):
    ...
    def _object_hook(self, dict_: dict) -> dict:
        for key, value in dict_.items():
            if key == "id" or "_id" in key:
                dict_[key] = UUID(value)
            elif isinstance(value, str) and (
                date := (
                    self._try_convert_date_to_string(value)
                    or self._try_convert_datetime_to_string(value)
                )
            ):
                dict_[key] = date
    ...
    def _try_convert_date_to_string(self, v: str) -> dt.date | None:
        try:
            return dt.date.fromisoformat(v)
        except ValueError:
            return None

    def _try_convert_datetime_to_string(self, v: str) -> dt.datetime | None:
        try:
